...
if [ -n "$DJANGO_SUPERUSER_USERNAME" ] && [ -n "$DJANGO_SUPERUSER_PASSWORD" ] && [ -n "$SIMULATED_ENVIRONMENT_URL" ] ; then
    (cd adaptive_policy_sync; python manage.py runscript dashboard_simulator --script-args 10 10 10 10; python manage.py runscript ise_ers_simulator --script-args 10 10 10; echo Use '$SIMULATED_ENVIRONMENT_URL/meraki/api/v1' for Meraki Dashboard\nUse '$SIMULATED_ENVIRONMENT_URL' for Cisco ISE)
fi
