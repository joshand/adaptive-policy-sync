...
def sync_ise_accounts(accounts):
    dt = make_aware(datetime.datetime.now())

    for sa in accounts:
        a = sa.iseserver
        print("ise_monitor::sync_ise_accounts::Resync -", a.description)
        ...
        if sgts and "error" not in sgts:
            for e in sgts:
                ...
                merge_sgts("ise", [thise["Sgt"]], sa.ise_source, sa)
            clean_sgts("ise", sgts, sa.ise_source, sa)

        if sgacls and "error" not in sgacls:
            for e in sgacls:
                thise = ers_get(a.ipaddress, "/sgacl/" + str(e["id"]), a.username, a.password)
                merge_sgacls("ise", [thise["Sgacl"]], sa.ise_source, sa)
            clean_sgacls("ise", sgacls, sa.ise_source, sa)

        if sgpolicies and "error" not in sgpolicies:
            for e in sgpolicies:
                thise = ers_get(a.ipaddress, "/egressmatrixcell/" + str(e["id"]), a.username, a.password)
                merge_sgpolicies("ise", [thise["EgressMatrixCell"]], sa.ise_source, sa)
            clean_sgpolicies("ise", sgpolicies, sa.ise_source, sa)
        ...
        for o in tags:
            if o.do_sync and not o.in_sync() and o.update_dest() == "ise":
                if sa.apply_changes:
                    m, u, d = o.push_config()
                    if m != "":
                        print("ise_monitor::sync_ise_accounts::tag API push", o.push_config())
                    ...
                    print("ise_monitor::sync_ise_accounts::tag needs API push", o.push_config())
        ...
        for o in acls:
            if not o.in_sync() and o.update_dest() == "ise":
                if sa.apply_changes:
                    m, u, d = o.push_config()
                    if m != "":
                        print("ise_monitor::sync_ise_accounts::acl API push", o.push_config())
                        ret = exec_api_action(m, u, d, headers)
                        o.last_update_data = ret
                        o.save()
                        sa.iseserver.force_rebuild = True
                    ...
                    print("ise_monitor::sync_ise_accounts::acl needs API push", o.push_config())
        ...
        for o in policies:
            if not o.in_sync() and o.update_dest() == "ise":
                if sa.apply_changes:
                    m, u, d = o.push_config()
                    if m != "":
                        print("ise_monitor::sync_ise_accounts::policy API push", o.push_config())
                        ret = exec_api_action(m, u, d, headers)
                        o.last_update_data = ret
                        o.save()
                        sa.iseserver.force_rebuild = True
                    ...
                    print("ise_monitor::sync_ise_accounts::policy needs API push", o.push_config())


def sync_ise():
    sync_list = []
    print("ise_monitor::sync_ise::Checking ISE Accounts for re-sync...")
    ...
    for db in dbs:
        print("ise_monitor::sync_ise::Force Rebuild -", db)
    ...
    for db in dbs:
        if db not in sync_list:
            print("ise_monitor::sync_ise::Out of Sync -", db)
    ...
    for s in ss:
        ...
        for db in dbs:
            if db not in sync_list:
                print("ise_monitor::sync_ise::Past sync interval -", dbs)
                sync_list.append(db)
                # sync_dashboard_accounts(dbs)

    for s in sync_list:
        if not s.sync_enabled:
            print("ise_monitor::sync_ise::Sync Disabled -", s)
            sync_list.remove(s)

    sync_ise_accounts(sync_list)
    print("ise_monitor::sync_ise::Done")
